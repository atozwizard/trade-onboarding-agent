...
<추출 항목>
-   **계약 금액 (contract_amount):** 계약의 총 규모 (예: "10만 달러", "5000만원"). 숫자와 단위를 포함하여 최대한 구체적으로 추출하고, 알 수 없으면 "추정 불가"로 기재하십시오.
-   **페널티 정보 (penalty_info):** 지연, 품질 이슈 등으로 발생할 수 있는 페널티 조항 (예: "5일 지연 시 일당 1% 페널티", "별도 페널티 조항 없음"). 최대한 구체적으로 추출하고, 알 수 없으면 "추정 불가"로 기재하십시오.
-   **예상 손실 (loss_estimate):** 금전적 또는 비금전적 예상 손실 규모 (예: "5천 달러", "1000만원", "생산 라인 중단"). 최대한 구체적으로 추출하고, 알 수 없으면 "추정 불가"로 기재하십시오.
...
{{
    ...
    "extracted_data": {{
        "contract_amount": "string", // 추출된 계약 금액 (예: "10만 달러", "5000만원", "추정 불가")
        "penalty_info": "string",    // 추출된 페널티 정보 (예: "5일 지연 시 일당 1% 페널티", "별도 페널티 조항 없음", "추정 불가")
        "loss_estimate": "string"    // 추출된 예상 손실 (예: "5천 달러", "1000만원", "생산 라인 중단", "추정 불가")
    }}
}}
```
<대화 내용>
