...
**데이터 소스:** 크롤링 CSV + ICC 무역용어집 PDF (Upstage Document Parse API, 283개 용어) → ChromaDB 벡터 DB 구축 → QuizAgent RAG 연결

**기술 스택:** FastAPI + ChromaDB(RAG) + Upstage Solar LLM + LangSmith
...
- **재시도 카운트 관리:** 재생성된 문제는 기존 `quiz_id`가 아닌 새 `quiz_id`를 발급받으므로, 재생성 직후 `retry_counts`에 초기값 1로 즉시 등록하여 무한 루프 방지
...
```json
{
  ...
  "agent_type": "quiz",
  "metadata": {"difficulty": null, "count": 5, "total_generated": 7}
}
```
...
주요 함수에 `@traceable` 데코레이터를 부착하여 LangSmith에서 전체 생성-검증 파이프라인을 추적합니다.
...
> `@traceable`이 동작하려면 `LANGSMITH_API_KEY`와 `LANGCHAIN_API_KEY`를 **모두** 환경변수로 설정해야 합니다.

### JSON 파싱 강건성

LLM이 JSON 앞뒤에 설명 텍스트를 붙이거나 코드블록(` ```json ... ``` `)으로 감싸는 경우가 있어, 단순 `json.loads()` 대신 정규식 기반 추출 로직을 사용합니다.

```
1. ` ```json ... ``` ` 코드블록 내 JSON 추출 시도
2. 코드블록 없으면 `[...] ` 또는 `{...}` 범위 regex 추출
3. 위 모두 실패 시 파싱 오류로 처리
```
